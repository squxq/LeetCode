...
class PQueue {
  ...
  enqueue(row: Row): void {
    this.elements.push(row);
    this.elements.sort((a: Row, b: Row) => {
      if (a.soldiers === b.soldiers) return a.index - b.index;
      return a.soldiers - b.soldiers;
    });
  }

  retrive(k: number): number[] {
    const result: number[] = [];
    for (let i: number = 0; i < k; i++) {
      result.push(this.elements[i]!.index);
    }
    return result;
  }
}
...
export function kWeakestRows(mat: number[][], k: number): number[] {
  ...
  for (let i: number = 0; i < mat.length; i++) {
    priorityQueue.enqueue({ index: i, soldiers: mat[i]!.lastIndexOf(1) + 1 });
  }
  return priorityQueue.retrive(k);
}
