 ...
 * @param {number[]} s
 * @returns {number}
 * produce the number of elements of g, i, such that s[i] >= g[i]
 * ASSUME: g and s are not ordered

 * Tests:
 * I: g = [1,2,3], s = [1,1] -> O: 1
 * I: g = [1,2], s = [1,2,3] -> O: 2
 * I: g = [10,9,8,7], s = [5,6,7,8] -> O: 2

 * Template: <list>
 function finContentChildren(g: number[], s: number[]): number {
   let ans: number = 0
 ...
 * Time Complexity: O(n*logn + m*logm), where n is g.length and m is s.length
 * Space Complexity: O(m + n)

 * Runtime: 71ms (97.96%)
 * Memory: 44.85MB (100.00%)
 */
export function findContentChildren(g: number[], s: number[]): number {
  const sortedG: number[] = g.sort((a: number, b: number) => a - b);
  const sortedS: number[] = s.sort((a: number, b: number) => a - b);

  let ans: number = 0;
  let j: number = 0;

  for (let i: number = 0; i < sortedS.length; i++) {
    if ((sortedS[i] as number) >= (sortedG[j] as number)) {
      ans++;
      j++;
    }
  }
